...
class Poligono_ActionServer(Node):
    ...
    def __init__(self, tipo = 0, distancia = 1.0):
        # Crea el nodo
        super().__init__('poligono_actionClient')
        # Crea el cliente de servicio y espera a conectarse
        self.client = ActionClient(self, Poligono, 'poligono')

    def enviar_peticion(self, longitud_lado = 1, num_lados = 3):
        msg = Poligono.Goal()
        # Define el contenido de la petición
        msg.longitud_lado = longitud_lado
        msg.num_lados = num_lados
...
def main(args=None):
    ...
    # Crea el nodo
    poligono_actionClient = Poligono_ActionServer()
    # Solicita los datos del movimiento deseado al usuario
    longitud_lado = float(input('Longitud del lado del polígono:\n'))
    num_lados = int(input('Número de lados del polígono:\n'))
    # Envía una petición al servidor desde el cliente
    poligono_actionClient.enviar_peticion(longitud_lado, num_lados)
    # Mantiene el cliente en ejecución
    rclpy.spin(poligono_actionClient)
